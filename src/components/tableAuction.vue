...
const searchAuctions = () => {
    if (!keyword.value) {
        ...
        filteredAuctions.value = listAuction.filter((auction) => auction.name.toLowerCase().includes(keyword.value.toLowerCase()));
    }
};
...
const getListAuction = async (page = 1) => {
    const res = await axios.get(`http://127.0.0.1:8080/secured/auction/list?page=${page}&size=${itemsPerPage}`, {
        headers: {
    ...
    <div>
        <div class="wrapper d-flex justify-content-between">
            <div class="form-outline">
                <input name="search-input" v-model="keyword" type="search" class="form-control" placeholder="Search" />
            </div>
            <router-link to="#" class="btn btn-primary"><i class="bi bi-plus-circle"></i> <span class="add-auction">Add Auction</span> </router-link>
        </div>
        <table class="table table-bordered">
            ...
            <tbody>
                <tr v-for="(data, index) in filteredAuctions" :key="index">
                    ...
                    <td>{{ data.status }}</td>
                    <td>
                        <button class="btn btn-success" title="Approve Auction">
                            <i class="bi bi-check-circle button-icon"></i>
                        </button>
                        <button class="btn btn-danger" title="Reject Auction">
        ...
        <div class="pagination-wrapper">
            ...
            <button class="btn btn-secondary" :disabled="!hasMoreData" @click="nextPage">Next</button>
        </div>
    </div>
</template>
